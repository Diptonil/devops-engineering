...
- **Container Registry**: Obsolete now. Store, manage and secure Docker container images. It sounds similar to Artifact Registry because that is Container Registry's successor. It is always recommended to go for Artifact Registry.
...
- **Assured Workloads**: This service provides a *setup to running more secure and compliant workloads on Google Cloud*.
    - During Assured Workloads setup, you create an environment and select your compliance regime. 
    - To gain more control over how data is encrypted, Google Cloud customers can use Cloud Key Management Service to generate, use, rotate, and destroy encryption keys according to their own policies. 



## Premium & Enhanced Support Services

These services are not available to all. Only those paying for the Premium or Enhanced Support may avail these services to use Cloud to the best of their advantages.
- **Active Assist Recommender**: A portfolio of intelligent tools *to actively assist you in managing complexities of the Cloud operations*.
    - Making improvements in the cloud environment with suggestions.
    - Prevent previous mistakes with clear analysis.
    - Intuitive troubleshooting tools help in understanding why something went wrong.

- **Cloud Support API**: Allows the *integration of Customer Care within the CRM* (Customer Relationship Manager). This is a unique offering by GCP that is otherwise not really noticed anywhere else. Available for both types of support.

- **Third-Party Technology Support**: This is a service built *to assist users in operating non-Google and open-source technologies* that are integrated with GCP. Available for both types of support.

- **Technical Account Advisor Service**: Available only for both types of support. It *provides proactive guidance and reactive support to help one succeed in their Cloud journey*.

- **Technical Account Manager**: *Trusted technical advisors that focus on operational rigor, platform health and architectural stability* for your organization. Only with Premium accounts.

- **Assured Support**: Enables us *to secure our **regulated workloads** and accelarate our path to running compliant workloads* on GCP.
    - To meet compliance requirements, Assured Support ensures that trained support personnel handle GCP workloads.
    - Regulated workloads are FedRAMP workloads, IL4 support services, et cetera.

- **Mission Critical Support**: *Assess and mitigate potential service disruptions for environments that are essential to an organization* and casue significant impact to organizations when disrupted. GCP analyses the current mode of operations and to prepare for this service, onboards a business to Mission Critical Support. This is expensive and only for use for extreme critical scenarios. Pros:
    - Drilling, testing and training for mission critical environments.
    - Customer-centric incident reporting.
    - Proactive monitoring and case generation.
    - War-room incident managements.

- **Event Management Services**: This is a support service for *planned peak events* such as a major sales event or new relaunch. The Customer Care partners with your team to make a plan and provide guidance throughout the event.

- **Training Credits**: *Qwiklabs credits that a business is offered by Google to train their personnel*.

- **New Product Previews**: *Previews of new Google products and support to start building solutions with them before the product even gets launched for maximum adoption speed*.


## Payment Services

- **Billing Accounts**: Defines *who pays for a given set of GCP services* and is connected to the payments profile. Sub-accounts under an account are also there.
    - One account may be mapped to one or more projects.
    - Has billing-specif roles with specific instructions as to who will pay for what resource used.
    - There is a self-serve online account and an invoiced offline account.

- **Payments Profile**: *Google-level resource managed by them not specific to GCP*. It is the payment service used to purchase anything via Google (at Play Store or pay for YouTube premium). It is responsible for storing credit card numbers, etc.
    - There is a business type account and an individual type account.

- **Cloud Billing IAM Roles**: Lets us realise *which users have administrative or cost viewing roles or functions in GCP* for a particular project.

- **Billing Health Checks**: They provide *personalized recommendations as to where the budgets can be minimised or which resource is being underutilised and can be sized down*. **Budget Alerts** is an important feature that alerts us when the budgets are being crossed.

- **Built-In Billing Reports**: This service is most frequently asked in exams. It has these sub-services:
    - **Billing Report**: *Interactive pricing explorer with graphs*. Cost forecasting, per-service charges, etc. can be accesed from here.
    - **Cost Table Report**:* Tabular breakdown of cost to analyse details on invoices*. It is used to provide invoice or statment cost details like taxes, project-level cost details mapped to projects and service IDs, etc. Nothing more.
    - **Cost Breakdown Report**: *At-a-glance waterfall overview of monthly charges and credits*. Less functionality than Billing Reports but more or less the same. Easier on the eyes.
    - **Pricing Report**: *GCP services SKU prices*. Prices are viewed up to the date. Effective discounts along with negotiated prices are shown here.